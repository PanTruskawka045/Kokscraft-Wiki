# Survival + Działki 1.21.8

Survival + Działki to unikalne połączenie klasycznej rozgrywki z systemem własnych terenów na najnowszej wersji gry.
Tryb ten oferuje wersję 1.21.8+, zapewniając dostęp do najnowszych bloków i mechanik. Gracze mogą rozwijać swoje
działki, zdobywać bogactwo poprzez handel i piąć się w rankingach, tworząc potężne sojusze.

- **Wersja gry:** 1.21.8+
- **Data startu 6 edycji:** 14 listopada 2025

![Jak zacząć](/assets/survival-dzialki/jak-zaczac.png)

## Jak zacząć?

Rozgrywka wygląda jak na klasycznym survivalu 1.21.8+. Twoim głównym celem jest zbieranie surowców, zarabianie pieniędzy
i zakup pierwszej działki. Wybierz swoją pracę, handluj z graczami, znajdź drugą połówkę i zostań milionerem.

> [!TIP]
> Skorzystaj z losowego teleportu na spawnie, aby znaleźć idealny i bezpieczny teren pod swoją pierwszą bazę!

## Praca - formy zatrudnienia

Jak zarabiać i jakie są dostępne zawody? Na naszym trybie możesz wybrać kilka ścieżek kariery. Każda z nich posiada
własne poziomy i zakresy obowiązków.

|                          DRWAL                           |                           GÓRNIK                           |                            PODRÓŻNIK                             |                           FARMER                           |                          ŁOWCA                           |
|:--------------------------------------------------------:|:----------------------------------------------------------:|:----------------------------------------------------------------:|:----------------------------------------------------------:|:--------------------------------------------------------:|
|    ![Drwal](/assets/survival-dzialki/praca-drwal.png)    |    ![Górnik](/assets/survival-dzialki/praca-gornik.png)    |    ![Podróżnik](/assets/survival-dzialki/praca-podroznik.png)    |    ![Farmer](/assets/survival-dzialki/praca-farmer.png)    |    ![Łowca](/assets/survival-dzialki/praca-lowca.png)    |
| ![Formy Drwal](/assets/survival-dzialki/formy-drwal.png) | ![Formy Górnik](/assets/survival-dzialki/formy-gornik.png) | ![Formy Podróżnik](/assets/survival-dzialki/formy-podroznik.png) | ![Formy Farmer](/assets/survival-dzialki/formy-farmer.png) | ![Formy Łowca](/assets/survival-dzialki/formy-lowca.png) |

## Bordery Światów

Granice światów (bordery) są zwiększane co jakiś czas wraz z rozwojem edycji.

| **ŚWIAT** |      **BORDER**      |
|:---------:|:--------------------:|
| OVERWORLD |    45000 x 45000     |
|  NETHER   |     6000 x 6000      |
|    END    | [red/]wkrótce![/red] |

## Działka i zarządzanie nią

[amber/]Survival PLUS DZIAŁKI! Za opłatą w walucie serwerowej możesz kupić działkę - rezerwację terenu tylko dla
siebie (chyba, że dodasz swoich sojuszników).[/amber]

### Lista komend

- `/dzialka dom` - teleportuje na działkę (koordynaty teleportu można zmieniać).
- `/dzialka info` - wyświetla informacje o działce, na której stoisz (nazwa, kordy, właściciel).
- `/dzialka kup` - otwiera GUI z możliwością zakupu bloku działki (kilka różnych wariantów).
- `/dzialka menu` - otwiera panel ustawień działki, na której stoisz (lub listę działek, jeśli
  jesteś poza nią).

### Rodzaje działek

|                      **RODZAJ DZIAŁKI**                      |                           **OPIS DZIAŁKI**                           |
|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
|       ![Mała](/assets/survival-dzialki/mala_icon.png)        |       ![Opis mała](/assets/survival-dzialki/mala_dzialka.png)        |
|    ![Średnia](/assets/survival-dzialki/srednia_icon.png)     |    ![Opis średnia](/assets/survival-dzialki/srednia_dzialka.png)     |
|       ![Duża](/assets/survival-dzialki/duza_icon.png)        |       ![Opis duża](/assets/survival-dzialki/duza_dzialka.png)        |
| ![Bardzo duża](/assets/survival-dzialki/bardzoduza_icon.png) | ![Opis bardzo duża](/assets/survival-dzialki/bardzoduza_dzialka.png) |
|    ![Ogromna](/assets/survival-dzialki/ogromna_icon.png)     |    ![Opis ogromna](/assets/survival-dzialki/ogromna_dzialka.png)     |

### Uprawnienia w panelu działki

Panel pozwala na szczegółowe zarządzanie twoim terenem:

|                           FUNKCJA W GUI                           |                               CO ROBI?                                |
|:-----------------------------------------------------------------:|:---------------------------------------------------------------------:|
|           ![Info](/assets/survival-dzialki/ss-info.png)           |     ![Opis Info](/assets/survival-dzialki/informacje_dzialka.png)     |
|            ![Dom](/assets/survival-dzialki/ss-dom.png)            |         ![Opis Dom](/assets/survival-dzialki/dom_dzialka.png)         |
|             ![TP](/assets/survival-dzialki/ss-tp.png)             |          ![Opis TP](/assets/survival-dzialki/tp_dzialka.png)          |
|         ![Gracze](/assets/survival-dzialki/ss-gracze.png)         |      ![Opis Gracze](/assets/survival-dzialki/gracze_dzialka.png)      |
|    ![Zablokowani](/assets/survival-dzialki/ss-zablokowani.png)    | ![Opis Zablokowani](/assets/survival-dzialki/zablokowani_dzialka.png) |
| ![Ustawienia](/assets/survival-dzialki/ss-ustawienia_dzialki.png) |  ![Opis Ustawienia](/assets/survival-dzialki/ustawienia_dzialka.png)  |
|          ![Nazwa](/assets/survival-dzialki/ss-nazwa.png)          |       ![Opis Nazwa](/assets/survival-dzialki/nazwa_dzialka.png)       |
|       ![Przedłuż](/assets/survival-dzialki/ss-przedluz.png)       |    ![Opis Przedłuż](/assets/survival-dzialki/przedluz_dzialka.png)    |
|    ![Odwiedzanie](/assets/survival-dzialki/ss-odwiedzanie.png)    | ![Opis Odwiedzanie](/assets/survival-dzialki/odwiedzanie_dzialka.png) |
|    ![Goście](/assets/survival-dzialki/ss-ustawienia_gosci.png)    | ![Opis Goście](/assets/survival-dzialki/ustawieniagosci_dzialka.png)  |

## Przydatne dodatkowe komendy

- `/pomoc` - wyświetla GUI z informacjami o trybie i listą komend.
- `/ustawienia` - otwiera GUI z ustawieniami konta.
- `/ignore nick` - możliwość zignorowania gracza.
- `/warp` - wyświetla GUI z dostępnymi warpami.
- `/rangi` - wyświetla GUI z opisem rang premium.
- `/prestiz` - wyświetla GUI systemu prestiżu.
- `/kosz` - bezpieczne usuwanie niepotrzebnych przedmiotów.

## Handel i Ekonomia

Rynek na serwerze rządzi się swoimi prawami - to gracze ustalają ceny przedmiotów! Serwer umożliwia handel na kilka
sposobów: wewnętrzny sklep, market, wymiany bezpośrednie oraz przelewanie waluty.

> [!WARNING]
> Na serwerze obowiązuje podatek [red/]5%[/red]!
> [orange/]Podatek obejmuje płatności poprzez /pay oraz czeki![/orange]

![Czek](/assets/survival-dzialki/czek.png)

### Komendy ekonomiczne

- `/market otworz` - otwiera menu serwerowego rynku.
- `/market wystaw cena` - wystaw przedmiot na rynek za określoną cenę na 24h.
- `/market dostawy` - schowek kupionych, wylicytowanych oraz wygaśniętych ofert.
- `/wymiana nick` - bezpieczna wymiana "coś za coś" (zapobiega oszustwom).
- `/czek kwota` - tworzy fizyczny czek pieniężny.
- `/aukcja wystaw kwota` - tworzy licytację na czacie (szybka sprzedaż).
- `/aukcja licytuj kwota` - podbija cenę licytowanego przedmiotu.

> [!TIP]
> Jeśli masz znajomego w potrzebie, przelej mu pieniądze używając komendy `/pay nick kwota`.

![Market](/assets/survival-dzialki/market.png)

## Małżeństwa

Na serwerze istnieje możliwość zawarcia związku małżeńskiego między dwoma graczami! Opcja ta daje liczne korzyści oraz
uprawnienia.

![Ślub Info](/assets/survival-dzialki/slub_info.png)

- `/slub info` - wyświetla informację o Twoim ślubie.
- `/slub invite nick` - wysyła zaproszenie do ślubu.
- `/slub rozwod` - bierze natychmiastowy rozwód.
- `/slub list` - wyświetla listę aktywnych małżeństw.

> [!IMPORTANT]
> Po wzięciu rozwodu musisz odczekać **24h** zanim ponownie weźmiesz ślub!

![Rozwód](/assets/survival-dzialki/slub_rozwod.png)

## Rangi i Skrzynie

Sprawdź ofertę w naszym sklepie: [SKLEP KOKSCRAFT](https://kokscraft.pl/sklep/dzialki)
Każda ranga posiada unikalne przywileje (sprawdź pod `/rangi`) oraz zestawy przedmiotów (podgląd w `/kit`).

| UPRAWNIENIA                        | [badge:KOKS:primary] | [badge:MVIP:green] | [badge:SVIP:cyan] | [badge:VIP:lime] | [badge:GRACZ:gray] |
|:-----------------------------------|:--------------------:|:------------------:|:-----------------:|:----------------:|:------------------:|
| Kolorowy nick na czacie            |         TAK          |        TAK         |        TAK        |       TAK        |        TAK         |
| Limit działek                      |          8           |         6          |         4         |        3         |         2          |
| Limit dodanych graczy do działki   |          25          |         20         |        16         |        12        |         8          |
| Limit domów                        |          10          |         7          |         5         |        3         |         2          |
| Dostęp do Dziennych Zadań Premium  |         TAK          |        TAK         |        NIE        |       NIE        |        NIE         |
| Pisanie na kolorowo                |         TAK          |        TAK         |        NIE        |       NIE        |        NIE         |
| /latanie (na działce)              |         TAK          |        NIE         |        NIE        |       NIE        |        NIE         |
| Limit przedmiotów na markecie      |          25          |         20         |        15         |        10        |         5          |
| Limit przedmiotów na aukcji        |          5           |         4          |         3         |        2         |         1          |
| Zarobek AFK na minutę              |          5$          |         4$         |        3$         |        2$        |         1$         |
| Szansa na Fragment Klucza AFK      |         50%          |        40%         |        30%        |       20%        |        10%         |
| Czas teleportacji                  |          5s          |         7s         |        8s         |        9s        |        10s         |
| Wyświetlanie przedmiotów na czacie |         TAK          |        TAK         |        TAK        |       TAK        |        NIE         |
| Wejście na pełny serwer            |         TAK          |        TAK         |        TAK        |       TAK        |        NIE         |
| /workbench                         |         TAK          |        TAK         |        TAK        |       TAK        |        NIE         |
| /hat                               |         TAK          |        TAK         |        TAK        |       TAK        |        NIE         |
| /gamma                             |         TAK          |        TAK         |        TAK        |       TAK        |        TAK         |
| /sellall                           |         TAK          |        TAK         |        TAK        |       NIE        |        NIE         |
| /repair                            |         TAK          |        TAK         |        TAK        |       NIE        |        NIE         |
| /feed                              |         TAK          |        TAK         |        TAK        |       NIE        |        NIE         |
| /enderchest                        |         TAK          |        TAK         |        NIE        |       NIE        |        NIE         |
| /heal                              |         TAK          |        TAK         |        NIE        |       NIE        |        NIE         |
| /repairall                         |         TAK          |        NIE         |        NIE        |       NIE        |        NIE         |

### Zestawy (Kity)

|                      |                        ZESTAWY                         |
|----------------------|:------------------------------------------------------:|
| [badge:GRACZ:gray]   | ![Kit Player](/assets/survival-dzialki/kit-player.png) |
| [badge:VIP:lime]     |  ![Kit Player](/assets/survival-dzialki/kit-vip.png)   |
| [badge:SVIP:cyan]    |  ![Kit Player](/assets/survival-dzialki/kit-svip.png)  |
| [badge:MVIP:green]   |  ![Kit Player](/assets/survival-dzialki/kit-mvip.png)  |
| [badge:KOKS:primary] |  ![Kit Player](/assets/survival-dzialki/kit-koks.png)  |



### Skrzynie (Crates)

W sklepie dostępne są klucze do specjalnych skrzyń:

|                         CO DROPIĄ?                          |
|:-----------------------------------------------------------:|
|   **[lime/]MAGICZNA SKRZYNIA[/lime]** - dostępna od 15.11   |
|   **[yellow/]KOKS SKRZYNIA[/yellow]** - dostępna od 15.11   |
| **[amber/]SZCZĘŚLIWA SKRZYNIA[/amber]** - dostępna od 15.11 |
|   **[red/]SKRZYNIA SPAWNERÓW[/red]** - dostępna od 15.11    |

## Topki

Na serwerze panuje zacięta rywalizacja! Najlepsi gracze otrzymują nagrody na starcie nowej edycji. Możecie rywalizować w
wielu kategoriach.
[MINECRAFT:&eTo co? Umówieni na wspólne wbijanie topek?]

## Eventy

Aby zapewnić rozrywkę i rywalizację, **co 2 godziny** uruchamiają się automatyczne eventy.

* **Warunek startu:** Minimum **30 graczy** na serwerze.
* **Nagrody:** Pierwsze trzy osoby otrzymują cenne upominki.

## Spawn

Spawn to serce serwera, gdzie zaczynasz swoją przygodę. Znajdziesz tu wszystkie niezbędne elementy:

* Losowy teleport (RTP).
* Informacje o borderach map.
* Portal do Endu.
* Tablice z Topkami najlepszych graczy.

> [!CAUTION]
> Spawn jest bezpieczną strefą (Non-PvP), ale tuż za linią PVP gracze mogą ze sobą walczyć! Uważaj, gdzie stawiasz
> kroki.
